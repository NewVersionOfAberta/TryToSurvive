	...
	: S_Base(System::Combat, l_systemMgr)
{
	...
	req.ClearBit((unsigned int)Component::Movable);
...
void S_Combat::Notify(const Message& l_message){
	if (!HasEntity(l_message.m_receiver)) { return; }//|| !HasEntity(l_message.m_sender)){ return; }
	EntityManager* entities = m_systemManager->GetEntityManager();
	...
	case EntityMessage::Being_Attacked:
		...
		//std::cout << "Health: " << victim->GetHealth() << std::endl;
		...
		if (attackerDirection == Direction::Left || attackerDirection == Direction::Right){ KnockbackVelocity.first = Knockback; }
		else{ KnockbackVelocity.second = Knockback; }
