...
public:
	...
	void AddBullet(sf::Vector2f pos, sf::Vector2f speed, int id, int owner);
	//void SetPlayerID(int l_playerId) { m_playerId = l_playerId; }

private:
	void CreateBullet(sf::Vector2f mousePos, sf::Vector2f entityPos, EntityManager* eMgr, int id, int eid);
	Bullets m_bullets;
	int m_currId;
	//int m_playerId;
};
