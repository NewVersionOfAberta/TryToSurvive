...
void EventManager::HandleEvent(ttsv::Event& l_event) {
	...
	for (auto& b_itr : m_bindings) {
		...
		for (auto& e_itr : bind->GetEvents()) {
			...
			if (e_itr.first != ttsvEvent) { continue; }
			if (ttsvEvent == EventType::KeyDown || ttsvEvent == EventType::KeyUp) {
			...
			//}
				...
				if (ttsvEvent == EventType::MouseWheel) {
					bind->m_details.m_mouseWheelDelta = l_event.mouseWheelScroll.delta;
				}
				else if (ttsvEvent == EventType::WindowResized) {
					bind->m_details.m_size.first = l_event.size.width;
					bind->m_details.m_size.second = l_event.size.height;
				}
